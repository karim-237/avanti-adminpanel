...
import type { products, product_reviews, User as PrismaUser } from '@prisma/client'

interface ReviewDetails extends product_reviews {
...
export default function ReviewList({
  ...
  const reload = async () => {
    try {
      const res = await getReviews({ productId: product.id, page: 1 })
      setReviews([...res.data])
      setTotalPages(res.totalPages)
    } catch (err) {
      toast({ variant: 'destructive', description: t('Error in fetching reviews') })
    }
  }
  ...
  const loadMoreReviews = async () => {
    ...
    const res = await getReviews({ productId: product.id, page })
  ...
  const handleOpenForm = async () => {
    ...
    const review = await getReviewByProductId({ productId: product.id, userId })
    if (review) {
      form.setValue('comment', review.comment || '')
      form.setValue('rating', review.rating || 0)
    }
    setOpen(true)
  }
  ...
  const avgRating = reviews.reduce((sum, r) => sum + (r.rating || 0), 0) / (reviews.length || 1)
      ...
      <div className='grid grid-cols-1 md:grid-cols-4 gap-8'>
        <div className='flex flex-col gap-2'>
          ...
          <div className='space-y-3'>
            <h3 className='font-bold text-lg lg:text-xl'>{t('Review this product')}</h3>
              ...
              <CardContent>
                <div className='flex space-x-4 text-sm text-muted-foreground'>
                  ...
                  <div className='flex items-center'>
                    ...
                    {review.created_at?.toISOString().substring(0, 10)}
                  </div>
                </div>
              </CardContent>
            </Card>
